...
def transform_target_encoding(texts):
    """
    func:将新闻title转化为target得格式,在正式得训练中使用
    :param texts:
    :return:
    """
    temp = []
...
def train_encoder_decoder(encoder, decoder, model, epochs=10, batch_size=2):
    x, y = [], []
    ...
    for epoch in range(epochs):
        ...
        while index < (len(Xtrain) - batch_size):
            ...
            # 训练生成器（不训练判别器）
            tmp = model.train_on_batch(x, y)[0]
...
encoder, decoder, model = build_model(en_input, de_input, decoder_state_inputs)

loss = train_encoder_decoder(encoder, decoder, model,epochs=100)
plt.plot(loss)
plt.show()
plt.savefig('train_loss.pdf')
# D_in = Input(shape=(None, num_input_tokens), name='dis_inputs')
...
# d_loss_real,d_loss_fake, g_loss = train_encoder_discriminator(GAN, D,epochs=100)
# for layer in GAN.layers:
#     if layer.name in ['model_1','model_4']:
#         print(layer.summary())

Xtrain, Xtest, Ytrain, Ytest = train_test_split(X[:50], Y[:50], test_size=0.1, random_state=42)
